...
contract Book5 {
    ...
    function checkOverdue(string _bookId) public constant bookIsExisted(_bookId) stringNotEmpty(_bookId) returns (bool, uint){
        if ((!bookStatusMap[_bookId].avail) && (now - bookStatusMap[_bookId].startBorrowTime > bookStatusMap[_bookId].maxBorrowDuration)) {
            returnBook(_bookId);
            return (true, 0);
        } else {
            return (false, bookStatusMap[_bookId].maxBorrowDuration + bookStatusMap[_bookId].startBorrowTime - now);
        }
    }
